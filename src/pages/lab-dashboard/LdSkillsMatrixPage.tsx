...
import { Plus, Edit, Trash2, Award, Users, Star, CheckCircle } from "lucide-react";
...
const proficiencyLevels = [
  ...
  { value: "intermediate", label: "Intermediate", color: "bg-amber-100 text-amber-800" },
  { value: "expert", label: "Expert", color: "bg-emerald-100 text-emerald-800" },
];
...
export default function LdSkillsMatrixPage() {
  ...
  const openEditDialog = (skill: any) => {
    ...
    setFormTechnicianId(skill.technician_id || "");
    setFormWorkTypeId(skill.work_type_id || "");
    setFormProficiency(skill.proficiency_level);
    setFormCertified(skill.certified || false);
    setFormCertDate(skill.certification_date || "");
    setFormNotes(skill.notes || "");
    setDialogOpen(true);
  };

  const handleSave = async () => {
    if (!formTechnicianId || !formWorkTypeId) {
      toast.error("Technician and work type are required");
      return;
    }
    const payload = {
      technician_id: formTechnicianId,
      work_type_id: formWorkTypeId,
      proficiency_level: formProficiency,
      certified: formCertified,
      certification_date: formCertDate || null,
      notes: formNotes,
    };
    try {
      if (editingSkill) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-2xl font-bold">{skills.filter(s => s.proficiency_level === "expert").length}</p>
        ...
        <Card>
          <CardContent className="flex items-center gap-4 py-4">
            ...
            <div>
              <p className="text-2xl font-bold">{skills.filter(s => s.certified).length}</p>
                ...
                <thead>
                  <tr>
                    ...
                    {activeWorkTypes.map(wt => (
                      <th key={wt.id} className="border p-2 bg-muted text-center font-medium text-sm">
                        {wt.name}
                      </th>
                    ))}
                  </tr>
                ...
                <TableBody>
                  {skills.map((s, idx) => (
                    ...
                    >
                      ...
                      <TableCell>
                        {s.certification_date ? format(new Date(s.certification_date), "MMM dd, yyyy") : "-"}
                      </TableCell>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="flex items-center gap-4">
              ...
              {formCertified && (
                <Input type="date" value={formCertDate} onChange={e => setFormCertDate(e.target.value)} className="w-40" />
              )}
            </div>
            <div className="space-y-2">
              <Label>Notes</Label>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={addSkill.isPending || updateSkill.isPending}>
