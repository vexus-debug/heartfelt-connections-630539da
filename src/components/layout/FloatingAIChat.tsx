  ...
  return (
    <>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed bottom-6 right-6 z-50 w-[360px] max-w-[calc(100vw-2rem)] h-[520px] max-h-[calc(100vh-3rem)] rounded-2xl border border-border/60 bg-background shadow-2xl shadow-foreground/10 flex flex-col overflow-hidden"
          ...
          >
              ...
              {isEmpty ? (
                  ...
                  <div className="grid grid-cols-2 gap-1.5 w-full">
                    {suggestions.map((s) => (
                      <button
                        ...
                        className="text-left text-[11px] px-2.5 py-2 rounded-lg border border-border/60 bg-card hover:bg-accent/50 hover:border-secondary/30 transition-all text-muted-foreground hover:text-foreground"
                      >
                        {s}
                      </button>
                    ))}
                ...
                <div className="space-y-3">
                  {messages.map((msg) => (
                    ...
                    >
                      {msg.role === "assistant" && (
                        <div className="h-6 w-6 rounded-md bg-secondary/10 flex items-center justify-center mr-1.5 mt-0.5 flex-shrink-0">
            ...
            <div className="px-3 pb-3 pt-1">
              <div className="flex items-end gap-1.5 bg-card border border-border/60 rounded-xl px-3 py-1.5 focus-within:ring-2 focus-within:ring-secondary/30 focus-within:border-secondary/50 transition-all">
                ...
                <Button
                  size="icon"
                  className="h-7 w-7 rounded-lg bg-secondary hover:bg-secondary/90 text-secondary-foreground shadow-sm"
                  onClick={() => handleSend()}
